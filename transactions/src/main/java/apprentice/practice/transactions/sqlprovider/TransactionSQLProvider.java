...
public final class TransactionSQLProvider {
  ...
  public static final String SELECT_EXISTS_BY =
      "SELECT EXISTS (SELECT 1 FROM transactions WHERE transaction_number=#{transactionNumber} AND status='TRY')";
  ...
  public static final String INSERT_TRANSACTION =
      "INSERT INTO "
          ...
          + "VALUES(#{transactionNumber}, #{transactionMoney}, #{transformerId}, #{transformeeId}, #{transactionTime}, "
          + "#{envelopeId}, #{envelopeMoney}, #{integralId}, #{integral}, #{status}, #{createAt}, #{updateAt})";
}
